...
> Revisión 13 (4 sep): el contenido real de las once soluciones y de las cinco páginas de práctica, sacado de los decks, fichas y one-pagers de cada Solution Manager (leídos uno a uno en el hub, incluidos los bundles de Claude Design) y de sus ficheros de estado, y cerrado con Susana en cincuenta decisiones (`docs/decisiones-revision-13.md`; el contenido campo a campo, en `docs/contenido-v3-<práctica>.md`). Va detrás de la 12 y la corrige donde chocan (bloque AI del mensaje). **Lo que cambia:** las once soluciones pasan a `vigente` con una nota plegada «en revisión por el área» y el `pendiente` real de cada SM; «Qué es», «A quién / la señal», «Por qué Entelgy», diferenciador, objeción principal, «Cómo abres» y primer paso con nombre y plazo en todas; kit completo (frases por dolor, pitch por rol, objeciones, preguntas de cualificación, material interno, dossier) y mensajes clave por dolor, que ahora se pintan en «Para prepararte»; unas treinta referencias nuevas o completadas, todas `confirmar_por_cuenta` salvo las que tienen documento público o autorización escrita (EJIE Educación, CNMV, Bankinter, UNED, MAPFRE, Entelgy en su casa, DGOJ, MICIU, H10); las piezas en su momento comercial según la doctrina del 15-jul (one-pager → primer contacto, deck → reunión, ficha → para dejar) en las cinco prácticas; siete piezas a `revisar` por cambios pendientes del dueño (los dos decks de vertical de SmartOPS, ficha y one-pager de Asistencia técnica, deck de la práctica SD, autodiagnóstico y ficha de Modernización); capacidades de práctica nuevas en Data Intelligence (cinco pilares), Smart Operations (el motor SmartOPS) y Digital Change (el oficio; `capacidades_ia` deja de pintarse); tres propuestas de portada nuevas (DI, SO, DC); precios visibles donde el SM los publica (portal interno); contactos con apellido, correo y título, y Luis Sanz y Cristina Aparicio como segundos contactos. **Decisiones que se apartan de lo recomendado:** la Oficina de `/entelgy` se llama ya «Oficina de Transformación y Gobernanza» (nombre de la web pública); las capturas del as-is de `/punto-de-partida` se quitan; Data Intelligence entra por el servicio gestionado de gobierno del dato («oficina del dato»), no por el assessment; «+3.000 clientes» y «−30% de inversión con IA» de Infraestructura crítica entran marcados en validación; Móstoles entra con nota; los cinco casos de éxito de SmartOPS se corrigen a `referencia` en vez de archivarse; Bahía de Bizkaia se crea vacía; Inverbis-GBTEC entra con ese nombre (verificado). **Cruces entre áreas que MA vería:** MAPFRE se cuenta una sola vez, bajo PreservIA, con las cifras de Alfredo; Puesto de trabajo remite a esa referencia. La Oficina de gobierno y orquestación de agentes (Carla) y la Oficina de Gobernanza Humana de la IA (Alfredo) conviven con nombres distintos hasta que lo cuadren. **Avisar antes del 10 (correos preparados):** Carmen, Carla, Jorge, Daniela, Amador y Alfredo, cada uno con lo que tiene que confirmar de su página.

> Revisión 14 (4 sep, tarde): la capa editorial. La 13 dejó el catálogo con contenido en las once soluciones y la v3 seguía viéndose como un panel de administración: todo al mismo tamaño, todo claro tras el hero, tarjetas iguales, chips en cada tarjeta. La v2 (`hipatia-oportunidades.html`) se veía mejor con la identidad equivocada porque estaba maquetada como una publicación; de ahí se importa la composición, no la paleta (`docs/analisis-visual-v2-v3.md`, `docs/brief-diseno-editorial.md`, mocks en `docs/mockup-editorial/`). **Cinco principios:** los dos segundos (dónde estoy, qué vendemos, qué le doy, a quién llamo, siempre en el mismo sitio); lo atractivo sale de la tipografía y del ritmo, nunca del color ni del adorno; el contraste tiene significado (navy para lo que dice Entelgy, blanco para lo que el comercial usa; una banda oscura por página además del hero); un acento para actuar (naranja, solo el CTA) y otro para orientar (morado); dos anatomías de tarjeta y ninguna más. **Nueve reglas** (E1–E9): escala de titulares (68/60/36/24), eyebrow con regla, una banda navy por página, cabeceras a dos columnas, numeración en fantasma solo donde hay secuencia, borde superior morado y una sola sombra dura por página, arcos concéntricos en los heros (el paréntesis del logo), chips solo para la excepción (el caso normal «sale al cliente · vigente» no lleva chip; las referencias pintan siempre su citabilidad), biblioteca con barra navy y tarjeta ligera. Se aplica a las cinco páginas antes del 10; si no llega, se recorta biblioteca, solución y práctica, en ese orden. **Entra en la misma revisión** lo que la v2 tenía en el relato corporativo y la v3 no llegó a construir (`docs/analisis-v2-v3-vision-corporativa.md`): en `/entelgy/`, «Por qué Entelgy» con la línea del deck v6 («Desplegar tecnología es fácil. Que mueva tu cuenta de resultados, no.»), el raíl de los cuatro pilares y los tres diferenciadores de MA como tres tarjetas; «Dónde entramos» con las cinco prácticas; las funciones que puede asumir la Oficina; las pruebas plegadas con las credenciales del deck v6 (20+ años · 2.000 · 100% capital propio · >95% renueva · ISO 27001/27701 · ENS Alto · SOC 2) y los sectores sin recuento; el deck corporativo al final. En las cinco prácticas, un plegable «Tres preguntas para abrir» con `discovery[]` (Data Intelligence cambia las suyas). **Biblioteca:** el vocabulario de `tipo` se cierra a nueve valores (Deck · One-pager · Ficha · Referencia · Guía de discovery · Guía interna · Plantilla · Herramienta · Archivo) con `subtipo` libre que se pinta y se busca; filtro fijo; dos índices estáticos (`/materiales/` con lo que sale al cliente, `/materiales/todo/` con las 112) para cumplir el peso sin perder el filtro «interno» que la 13 había dejado sin uso. **Dueños** con nombre y apellido en todo el portal, resueltos por el build desde `personas.json` (los JSON siguen con el nombre de pila). **Remates de la 13:** `material_interno_nota` (precios internos) se pinta en «Para prepararte»; los dos mapas de pains de SmartOPS con enlace; Barlow Condensed cargada con `<link>` en cabecera en vez de `@import`. **Decisiones que se quedan:** titular de portada (revisión 6); cajas de solución que envuelven el texto (11b); Data Intelligence en portada con una caja «una sola solución». **Para el 8 con MA:** el copy nuevo de `/entelgy/` (tesis, diferenciadores, credenciales, sectores, funciones de la Oficina) y la frase D3 frente a la del deck.

> Revisión 15 (4 sep, noche): las fuentes, desde el portal. La 14 dejó Barlow Condensed, Roboto y JetBrains Mono cargando desde Google Fonts con `<link>` en cabecera, y el lunes 8 el portal se enseña desde la red de Entelgy, que no sabemos si deja salir a `fonts.googleapis.com`; si no lo deja, MA lo ve en Arial. Las tres familias pasan a servirse desde `/assets/fonts/` (woff2, subconjunto latin, ocho pesos, licencia OFL 1.1, traídas con los paquetes de fontsource y copiadas por un script repetible), con `@font-face` al principio de `styles.css`, un `preload` de la de titulares y ninguna dependencia externa; `medir.js` vuelve al criterio estricto de cero errores de consola. Las fuentes no cuentan en el peso de página (HTML + CSS + JS). Se regeneran todas las capturas de `docs/medicion/` (las de prácticas y soluciones eran del día 3 y enseñaban la v3 de antes de la 14) y el PDF de portada, ya con la fuente buena. Cuatro remates: cabecera a dos columnas con eyebrow en el bloque de la Oficina de `/entelgy/` (E4); las tres notas de uso de Modernización que repetían «dueño: Jorge» pierden ese arranque (el chip y el apellido ya lo dicen); el panel de filtros de Materiales se pliega en un «Filtros» por debajo de 768 px; los dos índices de Materiales se ordenan por estado (vigente, revisar, pendiente), práctica y tipo, para que la biblioteca no abra con una pieza pendiente sin enlace y otra en revisión. **Se deja para después del 8:** los carriles vacíos de «Material común» en Smart Operations y Digital Change (decisión de datos) y el índice lateral de la práctica a 390. **Correos a los SM:** salen después del 8, con el enlace definitivo.

> Revisión 16 (5 sep): la primera vuelta de Susana por la build de la 14 (`docs/notas-susana-revision-16.md`). **Smart Operations** se reescribe desde el deck global de Amador («SmartOPS · Resumen Ejecutivo Global 2026»): la práctica es el motor de cinco piezas (N0–N3, dimensionamiento sobre datos, reporting Power BI, IA en producción, gobierno ISO/ENS) servido a dos territorios, no el puesto de trabajo; cinco capacidades, una línea de pruebas con fuente y un bloque nuevo, «Lo que presiona al CIO» (cinco fuerzas con autor, obra y año, y cuatro relojes; la sexta, atribuida a Gartner sin informe, se queda fuera), que por ahora solo tiene esta práctica (bloque opcional; las otras cuatro lo tendrán después del 10 desde sus decks). **La propuesta** de cada solución deja de ser un ladrillo con la maqueta (regla E10, 72 caracteres por línea; dos columnas; el diferenciador como cita sobre slate; la objeción como diálogo «Te dirán / Respondes»; la apertura junto al primer paso); la parte de datos (piezas, señales y pruebas como listas) va después del 10 con vista previa por solución. **Llévatelo**: cada pieza distingue presentar (en pantalla), enviar (PDF) y adaptar (PPT) con el campo `descargas[]`, y donde no hay fichero dice a quién pedírselo; arranca vacío y se rellena con lo que digan los SM después del 8; se prueba la impresión a PDF de una ficha y un one-pager. **Contactos** pasa de listado a directorio: `personas.json` con una entrada por persona (título, correo, teléfono, Teams, foto), cada solución con contacto comercial y técnico, una tabla «A quién llamo» por solución con el hueco del técnico a la vista y con dueño, y ocho tarjetas de persona; los datos que faltan (teléfonos, títulos, técnicos, fotos) se piden a los SM después del 8. **Carriles**: cuando una práctica no tiene pieza común para un momento, el carril toma la mejor pieza de sus soluciones. **Decidido:** todo antes del 8; si no llega, se recorta Llévatelo, carriles y maqueta de la propuesta, en ese orden.

> Revisión 17 (5 sep): remates de la 16, vistos en sus capturas. En «Lo que presiona al CIO» los ámbitos pasan a una palabra (Puesto · Cloud · Caídas · Regulación · IA), la unidad de la primera cifra se va al texto y el eyebrow de esas cinco tarjetas va sin regla, para que ninguna palabra se parta; «Qué cubre» de Smart Operations se acorta a setenta palabras para que las tres tarjetas midan lo mismo; la nota de pie de Contactos deja de hablar del calendario interno; y las diez notas del primer paso cierran con nombre y apellido («Alcance con Amador Sobrino»), sin «quick wins». La 15 y la 16 quedan verificadas desde el clon (`5949393`): fuentes servidas desde el portal sin ninguna carga externa, deck corporativo fijo al frente de la biblioteca (decisión de Susana ante la contradicción de AY.4: el deck está en «revisar»), filtros plegados en móvil, Smart Operations con el motor, la propuesta a dos columnas, Llévatelo, el directorio de contactos y los carriles. Tres decisiones de Código que se quedan: medir.js mide las páginas sueltas por su fichero (antes daban 404), las dos fichas de julio de Modernización cargan sus fuentes en local y llevan favicon, y quedan fuera del contraste del criterio 8 (el medidor no lee sus bandas oscuras y da falsos positivos); el autodiagnóstico queda fuera del criterio 8 entero, como del canon. La impresión a PDF de una ficha y un one-pager sale limpia (`docs/medicion/prueba-pdf/`): «Guardar como PDF» es viable para las piezas de una o dos hojas. `/materiales/todo/` está a 147 KB: si otra revisión suma estilos, salen del índice completo las piezas de archivo y legacy.

> Titular de portada (5 sep): pasa a «Cuando la tecnología no basta, Entelgy responde.». Sustituye al de la revisión 6 («Entelgy responde de que tecnología, procesos, personas y cultura avancen juntos hasta el resultado.»), que ocupaba seis líneas; el nuevo entra en tres. Deja sin efecto la línea «titular de portada (revisión 6)» de las «decisiones que se quedan» de las revisiones 12 y 14. El subtitular no cambia.

> Revisión 18 (5 sep, rama `claude/hipatia-v4-asistente`): la v4 es la v3 más «Pregunta a Hipatia», una pestaña en la esquina inferior derecha de todas las páginas que abre un panel y responde en frases con lo que hay en `data/`: qué enseñar para una práctica y un sector, a quién llamar, qué es una solución. Sin modelo, sin red ni clave: un índice ligero que emite el build (sin notas de uso ni precios), tres intenciones por patrón y plantillas fijas; cuando no tiene nada, lo dice. El índice y `pregunta.js` se cargan bajo demanda —al abrir el panel, no con la página— (Pregunta a Hipatia: índice 45 KB · pregunta.js 16 KB · los dos bajo demanda), así que no cuentan en el peso de página. Vive solo en la preview de la rama; el martes 9 se decide si entra en la demo del 10. El casado de palabras (revisión 19b) es por prefijo compartido: dos casan si comparten un prefijo de al menos cinco letras que cubre la más corta salvo, como mucho, sus dos últimas; las de menos de cinco casan enteras (así «corporativa» casa «corporativo» pero «presentación» no casa «PreservIA»). La prueba de aceptación es `data/preguntas.json`, escrito por Susana.
