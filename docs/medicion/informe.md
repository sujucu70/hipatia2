# Medición · Hipatia v3

> Generado por `medir.js` el 2026-09-03. Rutas medidas: 114.

## Criterios de aceptación (§6.8)

| # | Criterio | Estado |
|---|---|---|
| 1 | Prueba de los cinco comerciales | ⚪ la hace Susana |
| 2 | Solución a ≤2 clics, material cliente a ≤3 | 🟢 (solución máx 1, material máx 2) |
| 3 | 5 prácticas y 11 soluciones, mismos bloques en orden | 🟢 |
| 5 | Uso/estado/dueño, citable⇒sign_off, sin enlaces rotos | 🟢 (rotos: 0) |
| 6 | Ninguna pantalla afirma agente/envío/integración | 🟢 |
| 7 | URL propia y buscador en todas | 🟢 |
| 8 | Contraste AA · 0 errores consola · <150 KB · 390 px | 🟢 (peso máx 150 KB) |
| 9 | Reconocible por quien usó hipatia2 | ⚪ cualitativo |

## Clics desde la portada

- Solución más lejana: **1** clic(s).
- Material para cliente más lejano: **2** clic(s).

| Ruta | Clics |
|---|---|
| /practicas/process-intelligence/process-mining/ | 1 |
| /practicas/process-intelligence/process-automation-ai/ | 1 |
| /practicas/software-development/modernizacion/ | 1 |
| /practicas/software-development/mantenimiento/ | 1 |
| /practicas/software-development/asistencia-tecnica/ | 1 |
| /practicas/data-intelligence/data-intelligence/ | 1 |
| /practicas/smart-operations/smartops-workplace/ | 1 |
| /practicas/smart-operations/smartops-infra/ | 1 |
| /practicas/digital-change/iability/ | 1 |
| /practicas/digital-change/preservia/ | 1 |
| /practicas/digital-change/ogh-ia/ | 1 |

## Peso por página (HTML + CSS + JS)

| Ruta | KB |
|---|---|
| /materiales | 150 |
| /modernizacion/materiales | 62 |
| /modernizacion/materiales | 46 |
| /practicas/software-development/modernizacion | 44 |
| /autodiagnostico/grupo-lantia | 43 |
| / | 38 |
| /practicas/digital-change | 34 |
| /practicas/software-development | 34 |
| /entelgy | 33 |
| /practicas/data-intelligence/data-intelligence | 33 |
| /practicas/data-intelligence | 33 |
| /practicas/process-intelligence | 33 |

(máximo 150 KB de 114 páginas; imágenes aparte)

## Navegador

Capturas a 1440 y 390 px (10 rutas, muestra) en `docs/medicion/`.

- Errores de consola: **0** 
- Rutas con posible bajo contraste: **0** (ninguna en la muestra)

